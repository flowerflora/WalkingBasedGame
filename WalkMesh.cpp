...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	// going based off of in class code
	//if we invert the line, we should get the other triangle
	auto f = next_vertex.find(glm::uvec2(start.indices.y,start.indices.x));
	//other triangle: y,x,f->second
	...
	if (f!=next_vertex.end()) {
		...
		//TODO
		end.weights = barycentric_weights(vertices[start.indices.y],vertices[start.indices.x],vertices[f->second],to_world_point(start));
		...
		glm::vec3 start_normal = normals[start.indices[0]];
		glm::vec3 end_normal = normals[end.indices[0]];
